...
void readFromFile() {
	...
	FILE *fp = fopen("HighScoreOfGame.txt" ,"r");
	int i=0;
	if(fp!= NULL){
...
void writeIntoFile() {						//To write high score on to file
	FILE *fp = fopen("HighScoreOfGame.txt" ,"w");
	int i=0;
	char temp[40];
	if(fp!= NULL){
		int n= point;
...
void levelOne() {
    //PlaySound(TEXT("D:\\app\\parvez\\parvez\\startSong.wav"),NULL, SND_FILENAME | SND_ASYNC);
    ...
    if(gameover)
    {
     ...
     // cout<<temp2<<endl;
        ...
        if(gameOverSoundFlag == false)
        {
          ...
          //  PlaySound(TEXT("startSong.wav"),NULL, SND_FILENAME | SND_ASYNC);
    ...
    {
        highScore.createHighScoreMenu(atoi(high)); // sub menu
    }
    else //Game started
    {
